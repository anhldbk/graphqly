...
class Schema {
  ...
  mutation(def){
    ...
    if(_.has(this._mutations, name)){
      throw new Error("Another mutation having the same name is already defined");
    }
  ...
  _resolveStructures(){
    const resolving = _.merge({}, this._ifaces, this._enums, this._inputs, this._types);
    const defs = {};
  ...
  _resolveMutations(){
    _.forEach(this._mutations, (mutation) => {
      // verify if its dependencies are resolved
      _.forEach(mutation._dependencies, (dependency) => {
        if(!_.has(this._structures, dependency)){
          throw new Error(`Unresolved dependency ${dependency} in mutation ${mutation._name}`);
        }
      });
    });
  }
  ...
  _resolveQueries(){
    _.forEach(this._queries, (query) => {
      // verify if its dependencies are resolved
      _.forEach(query._dependencies, (dependency) => {
        if(!_.has(this._structures, dependency)){
          throw new Error(`Unresolved dependency ${dependency} in query ${query._name}`);
        }
      });
    });
  }
  ...
  build() {
    ...
    let structures = _.values(this._structures).map((structure) => structure.toString());
